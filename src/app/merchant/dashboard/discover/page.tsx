...
export default function MerchantDiscoverPage() {
  // State
  ...
  // 1. Fetch Merchant Products and Discover Collection
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');

      // Parallel fetch: Products, Discover Items, Categories
      const [prodRes, discRes, catRes] = await Promise.all([
        merchantService.getProducts(1, 100),
        merchantService.getDiscoverItems(1, 100),
        merchantService.getCategories(1, 100),
      ]);
  ...
  const handleSaveSelection = async () => {
    try {
      setSaving(true);
      const res = await merchantService.syncDiscoverProducts(selectedProductIds);
  ...
  const handleToggleItemStatus = async (item: DiscoverItem) => {
    try {
      const res = await merchantService.toggleDiscoverStatus(item._id);
      if (res.status !== false) {
  ...
  const handleDeleteItem = async (itemId: string, productId: string) => {
    try {
      const res = await merchantService.deleteDiscoverItem(itemId);
      if (res.status !== false) {
  ...
  const handleSaveEditItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await merchantService.updateDiscoverItem(editingItem._id, payload);
  ...
  return (
    <div className="merchant-discover-page">
      {/* Global CSS for Clean Gray Placeholders */}
      <style dangerouslySetInnerHTML={{
        __html: `
          .merchant-discover-page input::placeholder,
          .merchant-discover-page textarea::placeholder,
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted mb-0">
            Select the specific products you want to feature in the "Discover latest collection" slider on your store homepage.
          </p>
        </div>
        <Button
          variant="success"
        ...
        <Col xs={12} sm={4}>
          <Card className="border-0 shadow-sm rounded-3">
            <Card.Body className="d-flex align-items-center gap-3">
              ...
              <div>
                <div className="text-muted small">Total Store Inventory</div>
          ...
          <Tabs defaultActiveKey="picker" id="discover-tabs" className="mb-4">
            ...
            >
              ...
              <Row className="g-3 mb-4 align-items-center">
                ...
                <Col xs={12} sm={6} md={4} className="d-flex justify-content-md-end gap-2">
                  ...
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <Col key={product._id} xs={12} sm={6} md={4} lg={3}>
                        <Card
                          className={`h-100 border transition-all ${
                            isSelected
