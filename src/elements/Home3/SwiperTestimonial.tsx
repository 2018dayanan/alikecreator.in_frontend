...
export default function SwiperTestimonial() {
    ...
    useEffect(() => {
        const fetchDiscoverProducts = async () => {
            try {
                const res = await ProductService.getPublicDiscover();
                const items = res.data || [];

                if (Array.isArray(items) && items.length > 0) {
                    const formatted = items.map((item: any) => {
                        const prod = item.productId || {};
                        const imgUrl = prod.image || (Array.isArray(prod.images) && prod.images[0]) || null;
                        const title = item.customTitle || prod.title || 'Featured Product';
                        const link = prod.externalLink || '/shop-list';

                        return {
                            id: item._id || prod._id,
                            image: imgUrl || IMAGES.ShopPorductPng1,
                            name: title,
                            price: prod.price,
                    ...
                    setSliderItems(formatted);
                }
            } catch (err) {
                console.error("Error fetching discover showcase slider products:", err);
            }
        };
            ...
            >
                {sliderItems.map((item, ind) => (
                    <SwiperSlide key={item.id || ind}>
                        <div className="about-box position-relative">
                            {item.badge && (
                                <span
                                    className="badge bg-warning text-dark position-absolute top-0 start-0 m-3 px-2 py-1 fw-bold shadow-sm"
                            ...
                            <div className="about-img">
                                {typeof item.image === 'string' ? (
                                    ...
                                    <Image src={item.image} alt={item.name} style={{ width: '100%', height: '340px', objectFit: 'cover' }} />
                                )}
                            </div>
                            <div className="about-btn">
                                <Link
                                    className="btn btn-white btn-md shadow-sm d-flex align-items-center justify-content-between px-3"
                                    href={item.link || "/shop-list"}
                                    title={item.name}
                                >
